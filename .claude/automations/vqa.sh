...
case "$screen_filter" in
  all) screens=(welcome all_green runs runs-waiting runs-flaky rate_limit_toast detail detail-pending failure failure-flaky watch watch-board log dispatch dispatch-picker workflows diff diff-inconclusive flakes flakes-clean palette help approvals runs-loading detail-loading failure-loading log-progress rerun-confirm caches caches-pressure caches-empty) ;;
