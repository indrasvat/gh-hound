...
- Score: per job `{flake_score: 0..1, evidence: [{run_id, attempt, kind, detail}], verdict: "flaky"|"suspect"|"clean"}` with documented thresholds (e.g., ≥2 attempt flips in window → flaky). The result carries a **top-level `status`**: `"flaky" | "suspect" | "clean" | "insufficient_data"` plus `sample_size` and `window` — this is the field agents and the exit code branch on. **Worst job verdict always wins**: an underfilled window with clear attempt flips is still `flaky`/`suspect` (exit 1); `insufficient_data` means ONLY that no non-clean verdict can be supported because the sample is below the documented minimum — never that evidence was discarded. Test pins this: underfilled-but-flaky exits `1`.
