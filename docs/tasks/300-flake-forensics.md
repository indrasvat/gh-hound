...
- Signals (per job name within workflow+branch, over a window of `flake_window` runs, default 50):
  ...
  2. **Cross-run flapping:** same head_sha or adjacent commits alternating fail/pass with no related file changes (compare API from 260).
  3. **Retry-action masking:** step logs matching known retry wrappers (`nick-fields/retry`, `Retrying in …` patterns) where a step eventually "succeeds" — surfaced as masked instability.
- Score: per job `{flake_score: 0..1, evidence: [{run_id, attempt, kind, detail}], verdict: "flaky"|"suspect"|"clean"}` with documented thresholds (e.g., ≥2 attempt flips in window → flaky).
- Pipe: `gh hound flakes [--workflow <w>] [--branch <b>] --no-tui --json` → scored jobs; exit `0` clean, `1` flaky jobs found, `2` API error, `3` insufficient history (window underfilled, verdict `insufficient_data`).
- Failure screen: when the failing job has a non-clean verdict, a flake panel shows `seen this one before: flaked 3 of last 50 runs` with evidence drill-down (`enter` on an evidence row jumps to that run's detail).
