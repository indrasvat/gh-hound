...
gh hound watch --group --no-tui                  # whole event group, NDJSON until the pack settles
...
`watch --group --no-tui` streams the selected run's whole event group (same `head_sha` + `event`) as NDJSON: one `{type:"run", ts, run_id, workflow, status, conclusion}` line per state transition, closed by a `{type:"summary", …, running, home, lost}` object once the pack settles. Exit `1` if any run is lost, `0` when the pack comes home. Events are run-level only — drill into a single run with `watch --json` for job/step detail. Rehearse with `--fake-scenario pack`.
