...
gh hound watch --group --no-tui --timeout 10m          # bounded for unattended agents (timed_out summary; exit 3 in-flight / 1 if lost)
...
Schema and fixtures live under `internal/render/testdata/`. Full agent docs are in [docs/agent-surface.md](docs/agent-surface.md); the repo-local skill handoff is [skills/gh-hound/SKILL.md](skills/gh-hound/SKILL.md) (installable with `npx skills add indrasvat/gh-hound`).
